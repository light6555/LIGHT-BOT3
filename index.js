...
async function starts() {
	...
	const mdata = await client.groupMetadata(anu.jid)
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `𝐎𝐩𝐚 @${num.split('@')[0]}\n𝐁ienvenido 𝐚l 𝐠𝐫𝐮𝐩𝐨 *${mdata.subject}*\n\n𝐥𝐞e las reglas 𝐝el 𝐠𝐫𝐮𝐩𝐨 𝐩𝐚𝐫𝐚 𝐧o 𝐬𝐞𝐫 eliminado wachin😈`
    ...
    •            @${num.split('@')[0]}was E j e c t e d
                      1 impostor restante   。　.
    　 　　。　　 　　　　ﾟ　　　.　      　　　.
    ,　　𝐀𝐯𝐢𝐬𝐨 𝐩𝐚𝐫𝐚 usteded 𝐪𝐮𝐞 𝐬𝐚𝐢𝐮 @${num.split('@')[0]} chau gordo ojala que munca regreses wachin😈👋`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					...
					levelnoton: '❬ X ❭ *leveling no ativado*',
					levelnol: '*error* 0 °-°',
					error: {
				...
				only: {
					group: '[❗] Este comando só pode ser usado en grupos! ❌',
					premium: '[❗] ESTE PEDIDO É SO PARA *USUÁRIOS PREMIUMS*',
      		mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD LIGHT BOT*',
					benned: 'Você para a banido do bot, por favor, contate o proprietário para retirar seu ban',
					ownerG: '[❗] Skiller? Este é um recurso especial para o LIGHT❌',
					ownerB: '[❗] Skiller? Este é um recurso especial para o LIGHT❌',
					...
					Badmin: ' [❗] Este comando solo pude ser usado quando o bot se torna administrador! ❌',
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["+33660217049@s.whatsapp.net"] // coloque seu numero
			const mod = [ownerNumber,"33660217049@s.whatsapp.net"]//seu numero
			const adminbotnumber = ["33660217049@s.whatsapp.net"]//seu numero
			const frendsowner = ["33660217049@s.whatsapp.net"]//seu numero
			const premium = ["33660217049@s.whatsapp.net","33660217049@s.whatsapp.net",] 
...
reply("oi posso ajudar")
	...
	if (messagesC.includes(".faustinho")){
			...
			switch(command) {
				...
				case 'addvip':  
					if (!isOwner) return reply(mess.only.ownerB)
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o LIGHT ou digite * # Daftarvip * para adquirir o acesso Premium!' ,text, { quoted: mek })
...
case 'take':
                    ...
                    exifst = getRandom('.webp')
                      googlelink = `https://wa.me/557877666?text=${prefix}menu`;
                    applelink = `https://wa.me/5008777?text=${prefix}menu`;
...
case 'figu':
				...
				case 'tts':
					...
					break
				case 'toimg':
					if (!isQuotedSticker) return reply('{ ❗ } *Marque a imagen*')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.png')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ Falla al converter adesivos en imagens ❌')
						buffer = fs.readFileSync(ran)
...
case 'antilink':
				...
				//INFO BOT
case 'owner':
                    client.sendMessage(from, {displayname: "LIGHT", vcard: vcard}, MessageType.contact, { quoted: mek})
...
case 'daftarvip': 
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CREADOR:* LIGHT\n*YOUTUBE:* https://youtube.com/c/LIGHT\n*WPP:* wa.me/+6591991655\n*INSTA:* @LIGHT.on\n\n\nEspero que te guste el bot 😈🚩️'})
					break
				//_IMAGENS
				case 'boanoite':
					memein = await kagApi.memeindo()
